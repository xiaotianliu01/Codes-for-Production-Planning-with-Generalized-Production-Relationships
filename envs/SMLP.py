...
class Env(object):
    ...
    def __init__(self, seed, data_pth, instance_id, lam, episode_max_step, thread_num=1, exp_name='Fixed_Production_Lead_Time'):
        ...
        self.eposide_max_steps = episode_max_step
        
        if(exp_name == 'Fixed_Production_Lead_Time'):
            from .Prod_Net_Fixed_Lead_Time import Production_Net
            self.prod_net = Production_Net(seed, data_pth, instance_id, lam)
        elif(exp_name == 'Clearing_Function_Frac'):
            from .Prod_Net_Clearing_Function import Production_Net
            self.prod_net = Production_Net(seed, data_pth, instance_id, lam, func_form = 'frac')
        elif(exp_name == 'Clearing_Function_Exp'):
            from .Prod_Net_Clearing_Function import Production_Net
            self.prod_net = Production_Net(seed, data_pth, instance_id, lam, func_form = 'exp')
        elif(exp_name == 'Clearing_Function_Poly'):
            from .Prod_Net_Clearing_Function import Production_Net
            self.prod_net = Production_Net(seed, data_pth, instance_id, lam, func_form = 'poly')
    ...
    def step(self, actions, denormalize = True):
        ...
        costs, eval_costs, self.prod_states, self.demand_states, infos = self.state_update(action)
    ...
    def state_update(self, action):
        ...
        new_prod_states, production = self.prod_net.update_prod_state(self.prod_states, Q, self.step_num, self.thread_num)
