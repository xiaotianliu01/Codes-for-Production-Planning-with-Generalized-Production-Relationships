...
class Env(object):
    ...
    def __init__(self, seed, data_pth, instance_id, lam, episode_max_step, thread_num=1, exp_name='Fixed_Production_Lead_Time'):
        ...
        if(exp_name == 'Flow_Shop'):
            from .Prod_Net_Flow_Shop import Production_Net
    ...
    def step(self, actions, denormalize = True):
        ...
        if(denormalize):
            sched_actions = self.softmax(sched_actions)
    ...
    def state_update(self, action, sched_actions):
        ...
        new_prod_states, production, scheduling_costs = self.prod_net.update_prod_state(self.prod_states, Q, sched_actions, self.step_num, self.thread_num)
        ...
        costs = np.stack(costs, axis=1) + scheduling_costs
        eval_costs = np.stack(eval_costs, axis=1) + scheduling_costs
