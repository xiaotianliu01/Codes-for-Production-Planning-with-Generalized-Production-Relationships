...
class Production_Net():
    ...
    def __init__(self, seed, data_pth, instance_id, lam):
        ...
        self.prod_state_dim = [2]*self.num_product
        self.num_poisson_prod_mean = [[self.demand_mean[i]*0.6, self.demand_mean[i]*0.4] for i in range(self.num_product)]
    ...
    def update_prod_state(self, old_s, Qs, sched, step_num, thread_num):
        ...
        production = []
        for i in range(self.num_product):
            max_finished_jobs = np.random.poisson(np.array(self.num_poisson_prod_mean[i]), (thread_num, self.prod_state_dim[i]))
            new_jobs = (Qs[:,i,np.newaxis]*sched[:,i,:]).astype('int')
            all_jobs = old_s[i] + new_jobs
            finished_jobs = np.min(np.stack([all_jobs, max_finished_jobs], axis=-1), axis=-1)
            new_s = all_jobs - finished_jobs
            production.append(np.sum(finished_jobs, axis=1))
            s.append(new_s)
